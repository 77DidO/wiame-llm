...
// SSE transport for Docker — clients connect via HTTP
async function main() {
  const app = express();
  ...
  app.listen(MCP_PORT, "0.0.0.0", () => {
    console.error(`wiame-llm MCP server listening on http://0.0.0.0:${MCP_PORT}`);
    console.error(`  SSE endpoint: http://0.0.0.0:${MCP_PORT}/sse`);
    console.error(`  vLLM backend: ${VLLM_BASE_URL}`);
  });
}
